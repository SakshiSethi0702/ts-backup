...
exports.getPopularTemplates = exports.templateDownloads = exports.getLatestTemplates = exports.featureTemplates = exports.getAllTemplates = void 0;
exports.createTemplate = createTemplate;
exports.deleteTemplate = deleteTemplate;
exports.getTemplates = getTemplates;
exports.getAllTemplatesByUserId = getAllTemplatesByUserId;
exports.getTemplateById = getTemplateById;
exports.getTemplateByTitle = getTemplateByTitle;
...
async function createTemplate(req, res) {
    ...
    try {
        ...
        const uploadFiles = async (files, folder) => {
            return Promise.all(files.map(file => (0, fileService_1.uploadFileToFirebase)(file, folder)));
        };
        // Initialize empty arrays for the uploaded URLs
        let sliderImageUrls = [];
        let previewImageUrls = [];
        let previewMobileImageUrls = [];
        let sourceFileUrls = [];
        ...
        if (Array.isArray(req.files)) {
            ...
            if (req.files.sliderImages) {
                sliderImageUrls = await uploadFiles(req.files.sliderImages, 'sliderImages');
            }
            if (req.files.previewImages) {
                previewImageUrls = await uploadFiles(req.files.previewImages, 'previewImages');
            }
            if (req.files.previewMobileImages) {
                previewMobileImageUrls = await uploadFiles(req.files.previewMobileImages, 'previewMobileImages');
            }
        ...
        await Promise.all([
            ...sliderImageUrls.map(url => server_1.default.sliderImage.create({ data: { imageUrl: url, templateId: newTemplate.id } })),
            ...previewImageUrls.map(url => server_1.default.previewImage.create({ data: { imageUrl: url, templateId: newTemplate.id } })),
            ...previewMobileImageUrls.map(url => server_1.default.previewMobileImage.create({ data: { imageUrl: url, templateId: newTemplate.id } })),
            ...sourceFileUrls.map(url => server_1.default.sourceFile.create({ data: { fileUrl: url, templateId: newTemplate.id } })),
        ]);
...
async function deleteTemplate(req, res) {
    ...
    try {
        ...
        await server_1.default.$transaction([
            // Delete related records in other models
            server_1.default.credit.deleteMany({ where: { templateId: id } }),
            server_1.default.sourceFile.deleteMany({ where: { templateId: id } }),
            server_1.default.sliderImage.deleteMany({ where: { templateId: id } }),
            server_1.default.previewImage.deleteMany({ where: { templateId: id } }),
            server_1.default.previewMobileImage.deleteMany({ where: { templateId: id } }),
            server_1.default.downloadHistory.deleteMany({ where: { templateId: id } }),
            // Delete the main template record
            server_1.default.template.delete({
                where: { id },
            }),
        ]);
        res.status(200).json({
...
async function getTemplates(req, res) {
    try {
        const { industryTypeIds, // Can be string or array
        templateTypeId, // Can be string or array
        softwareTypeIds, // Can be string or array
        subCatId, // Single string
        isPaid, priceRanges, // Can be string or array
        search, page = 1, limit = 12, sortBy, } = req.query;
        ...
        const [templates, totalTemplates] = await Promise.all([
            server_1.default.template.findMany({
                where: {
                    ...filters,
            ...
            server_1.default.template.count({
                where: {
                    ...filters,
                    ...(templateTypeId ? { templateTypeId: { in: handleArrayInput(templateTypeId) } } : {}),
                    ...(subCatId ? subCategoryFilter : {}),
...
const getAllTemplates = async (req, res) => {
    try {
        const latestTemplates = await server_1.default.template.findMany({
            // include:{
            //   templateType:true
            // }
            select: {
...
exports.featureTemplates = featureTemplates;
/**
 * Fetches the latest templates from the database.
 *
 ...
 */
const getLatestTemplates = async (req, res) => {
    try {
        const latestTemplates = await server_1.default.template.findMany({
            orderBy: {
                createdAt: 'desc',
 ...
 */
const templateDownloads = async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const templateData = await server_1.default.template.findFirst({
            where: { id: id }
        });
        if (userId) {
            ...
            // Send email notification
            const downloadsCount = await server_1.default.downloadHistory.count({
        ...
        await server_1.default.downloadHistory.create({
            data: {
                templateId: id,
...
exports.templateDownloads = templateDownloads;
/**
...
const getPopularTemplates = async (req, res) => {
    try {
        const popularTemplates = await server_1.default.template.findMany({
            orderBy: {
                downloads: 'desc',
            },
            take: 10,
...
async function getTemplateById(req, res) {
    const { id } = req.params;
    try {
        const template = await server_1.default.template.findUnique({
            where: { id },
            include: {
                credits: true,
                sliderImages: true,
 ...
 */
async function getTemplateByTitle(req, res) {
    ...
    try {
        const results = await server_1.default.template.findMany({
            where: {
                AND: [
...
async function updateTemplate(req, res) {
    ...
    try {
        ...
        const existingTemplate = await server_1.default.template.findUnique({
            where: { id },
            include: {
                sliderImages: true,
                previewImages: true,
        ...
        // }
        const { title, price, description, industry, templateTypeId, softwareTypeId, version, isPaid, seoTags, techDetails } = req.body;
        ...
        const result = await server_1.default.$transaction(async (prisma) => {
